...
<div class="container mx-auto py-10 max-w-lg">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-bold text-primary">Tambah Category</h1>
    </div>
    @if(session('success'))
        <div class="mb-4 px-4 py-2 bg-green-100 border border-green-400 text-green-700 rounded shadow">{{ session('success') }}</div>
    @endif
    <form method="POST" action="{{ route('admin.news-categories.store') }}" class="bg-white p-8 rounded shadow border space-y-5">
        @csrf
        <div>
            <label class="block mb-1 font-semibold">Title</label>
            <input type="text" name="title" class="w-full border px-3 py-2 rounded focus:ring-primary focus:border-primary" required value="{{ old('title') }}">
            @error('title')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block mb-1 font-semibold">Slug</label>
            <input type="text" name="slug" class="w-full border px-3 py-2 rounded focus:ring-primary focus:border-primary" required value="{{ old('slug') }}">
