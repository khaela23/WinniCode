...
<div class="container mx-auto py-10 max-w-lg">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-bold text-primary">Edit Banner</h1>
    </div>
    @if(session('success'))
        <div class="mb-4 px-4 py-2 bg-green-100 border border-green-400 text-green-700 rounded shadow">{{ session('success') }}</div>
    @endif
    <form method="POST" action="{{ route('admin.banners.update', $banner->id) }}" class="bg-white p-8 rounded shadow border space-y-5">
        @csrf
        @method('PUT')
        <div>
            ...
            <select name="news_id" class="w-full border px-3 py-2 rounded focus:ring-primary focus:border-primary" required>
                ...
                @foreach($news as $item)
                    <option value="{{ $item->id }}" @selected(old('news_id', $banner->news_id) == $item->id)>{{ $item->title }}</option>
                @endforeach
            </select>
